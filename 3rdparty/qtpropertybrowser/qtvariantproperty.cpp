...
public:
    ...
    void slotCompleterChanged(QtProperty *property, const QStringList &strs);
    ...
    const QString m_completerAttribute;
};
...
QtVariantPropertyManagerPrivate::QtVariantPropertyManagerPrivate() :
    ...
    m_completerAttribute(QLatin1String("completer"))
{
}
...
void QtVariantPropertyManagerPrivate::slotCompleterChanged(QtProperty *property, const QStringList &strs) {
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
        emit q_ptr->attributeChanged(varProp, m_completerAttribute, QVariant(strs));
}
    ...
    : QtAbstractPropertyManager(parent), d_ptr(new QtVariantPropertyManagerPrivate)
{
    ...
    d_ptr->m_typeToAttributeToAttributeType[QMetaType::QString][d_ptr->m_completerAttribute] =
    ...
    // add completer
    connect(stringPropertyManager, SIGNAL(completerChanged(QtProperty*,QStringList)),
            this, SLOT(slotCompleterChanged(QtProperty*,QStringList)));
    // DatePropertyManager
...
QVariant QtVariantPropertyManager::attributeValue(const QtProperty *property, const QString &attribute) const
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        if (attribute == d_ptr->m_completerAttribute)
            return stringManager->completerStrings(internProp);
        return QVariant();
    } else if (QtDatePropertyManager *dateManager = qobject_cast<QtDatePropertyManager *>(manager)) {
        if (attribute == d_ptr->m_maximumAttribute)
        ...
        const QString &attribute, const QVariant &value)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        if (attribute == d_ptr->m_completerAttribute)
            stringManager->setCompleterList(internProp, qvariant_cast<QStringList>(value));
        return;
    } else if (QtDatePropertyManager *dateManager = qobject_cast<QtDatePropertyManager *>(manager)) {
        if (attribute == d_ptr->m_maximumAttribute)
