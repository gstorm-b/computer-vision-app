...
/**
 * @file fanuc_irvision_board.h
 * @brief FanucIRvisionBoard — Fanuc iRVision-style dot grid calibration board.
 */
 ...
 * @brief Fanuc iRVision-style dot grid calibration board (concrete CalibrationBoard).
 *
 * Pattern: R × C grid of dots with 4 visual roles:
 ...
 *   - Coord dots  : 3 larger solid dots near centre encoding +X (two dots right of centre)
 *                   and +Y (one dot above centre)
 *
...
public:
    /**
     * @struct Params
     * @brief Geometry and detection parameters for a FanucIRvisionBoard instance.
     */
    struct Params {
        ...
        /**
         * @brief Checks internal consistency of the parameter set (odd/min row-col counts,
         *        positive spacings, and the normal < coor dot-size < spacing ordering).
         * @param[out] errorOut if non-null and validation fails, receives a description of the
         *                      first violated rule
    ...
    /**
     * @brief Constructs the board with the given params.
     * @param[in] params board geometry/detection parameters (defaults to standard 11×11, 15 mm layout)
    ...
    /**
     * @brief Synthesizes the board as a grayscale dot-grid image (white background, black dots
     *        per DotRole, using the configured dot sizes and margin).
     * @param[in] pixelsPerMm rasterization scale used to convert the mm-based Params into pixels
    ...
    /**
     * @brief Detects the dot grid in `image`: binarizes, extracts near-circular blobs, identifies
     *        the 4 orientation markers (centre + 3 coord dots) by geometry, then projects the
     *        expected grid and matches each blob to its nearest cell.
     * @param[in]  image              input image (colour or grayscale)
     * @param[out] imagePoints        on success, one detected point per grid cell in row-major order
     * @param[out] cornerImagePoints  optional; the 4 corner dot positions (TL, TR, BR, BL) when non-null
     * @param[out] debugOverlay       optional; a BGR visualisation of the detected markers/grid/corners when non-null
    ...
    /**
     * @brief Sets the binarization threshold used by detect(), clamping to the valid range.
     * @param[in] threshold desired threshold; values < 0 collapse to -1 (auto/Otsu), values > 255 clamp to 255
     */
    ...
    /**
     * @brief Reads the Params from a JSON document produced by toJson().
...
private:
    ...
    /**
     * @enum DotRole
     * @brief Visual role a grid dot can play: plain fill dot, one of the 8 target rings,
