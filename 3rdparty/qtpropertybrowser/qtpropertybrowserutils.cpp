	...
	m_button(new QPushButton(this))
{
    ...
    connect(m_lineedit, &QLineEdit::editingFinished, this, &QtStringEdit::text_edit_finshed);
    ...
    QCompleter *completer = new QCompleter(m_strs, m_lineedit);
...
void QtStringEdit::setCompleter(QStringList strs) {
    m_strs = strs;
    m_strs.detach();
    QCompleter *completer = m_lineedit->completer();
    QStringListModel *model = qobject_cast<QStringListModel*>(completer->model());
    if (model) {
        model->setStringList(m_strs);
    }
}

void QtStringEdit::text_edit_finshed() {
    if (m_strs.isEmpty()) {
        emit textEditingFinished(m_lineedit->text());
        return;
    }

    QString currentText = m_lineedit->text();
    m_lineedit->blockSignals(true);
    if (!m_strs.contains(currentText)) {
        m_lineedit->setText(m_strs.contains(m_initialvalue) ? m_initialvalue : "");
    }
