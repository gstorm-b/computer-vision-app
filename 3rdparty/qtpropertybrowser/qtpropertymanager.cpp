...
/**
 * @brief QtStringPropertyManager::completerStrings
 * @param property
 * @return
 */
QStringList QtStringPropertyManager::completerStrings(const QtProperty* property) const
{
    return getData<QStringList>(d_ptr->m_values, &QtStringPropertyManagerPrivate::Data::completerList, property, QStringList());
}
...
/*!
 * \brief setCompleterList : set completer for edit widget
 * \param property
 * \param strs
 */
void QtStringPropertyManager::setCompleterList(QtProperty *property, const QStringList& strs)
{
    if (strs.empty()) {
        return;
    }
    ...
    data.completerList = strs;

    it.value() = data;

    emit completerChanged(property, data.completerList);
}

void QtStringPropertyManager::setInitialString(QtProperty* property, const QString& val) {
    const QtStringPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
