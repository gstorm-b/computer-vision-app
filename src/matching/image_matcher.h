 ...
 * @brief ImageMatcher — gradient-direction edge template-matching engine and its MatchResult
 *        output type.
 */
...
/**
 * @struct MatchResult
 * @brief Snapshot of one ImageMatcher::matching() run: the matched objects, the annotated
 *        output image, timing, and picking-availability stats.
 */
struct MatchResult {
...
/**
 * @class ImageMatcher
 * @brief Edge-based template matching engine: runs an OpenCV pyramid, coarse-to-fine,
 *        rotation-searching shape/edge match for every registered pattern against a source
 *        image, then filters, sorts, and annotates the results.
 *
 * Algorithm config lives in the group's MatchGroupConfig::typeConfig (EdgeMatchConfig),
 * shared by every pattern in the group. Runtime search parameters (max_pos_num, max_overlap)
 * are kept here as they govern the search over the whole image, not a single pattern.
 */
class ImageMatcher {
public:
    ...
    /**
     * @brief Replaces the source image (clone of `image`), then runs the same pipeline
     *        as the other matching() overload.
     * @param[in] image              source image to clone and match against
     * @param[in] boundingBoxChecking accepted but not read by the current implementation
     * @param[in] objectsNum         stop scanning once this many objects are matched and
     *                               at least one is possible-to-pick; negative disables
     * @param[in] usingRoi           crop the source image to the matching ROI before searching
     * @param[in] usingConditionRoi  classify matched objects against the condition ROI
     */
    void matching(cv::Mat& image, bool boundingBoxChecking, int objectsNum, bool usingRoi, bool usingConditionRoi);
    /**
     * @brief Core matching pipeline: grayscales + thresholds + finds contours in the current
     *        source image, shortlists contours whose area falls in each pattern's expected
     *        range, runs MatchEdge per candidate/pattern, resolves picking/collision geometry
     *        and robot-pickability, sorts, and rewrites match_result (including the annotated
     *        match_result.Image).
     * @param[in] boundingBoxChecking accepted but not read by the current implementation
     * @param[in] objectsNum         stop scanning once this many objects are matched and
     *                               at least one is possible-to-pick; negative disables
    ...
    /**
     * @brief Runs the coarse-to-fine pyramid, rotation-searching edge match of one pattern
     *        against `imgEdge`: coarse search at the top pyramid layer, then descends the
     *        pyramid refining position/angle (with optional sub-pixel estimation at layer 0),
     *        filters by score and by rotated-rect overlap, and builds one MatchedObject per
     *        surviving candidate.
     * @param[in]  imgEdge   image region to search (already cropped to the candidate area)
     * @param[in]  pattern   model pattern to match; must be learned (isPatternLearned())
     * @param[out] matchObjs filled with one MatchedObject per accepted match (points/angle/score
     *                       set; parent set to `pattern`, not yet translated to full-image coords)
     * @return true if at least one match scored above the pattern's minimum score; false if the
     *         pattern is unlearned, images are incompatible in size, the group's algorithm config
     *         is not EdgeBased, or no candidate reached the score threshold
     */
    ...
    /**
     * @brief Whether the robot can actually pick this matched object. Sequential steps:
     *          1. convert the object's image position/angle to a world pick pose,
     *          2. attach the pattern's picking offset so the pose tested is the one the
     *             robot will really be commanded to, not the bare match centre,
     *          3. solveAll IK to confirm the composed pose is reachable,
     *          4. (only if the robot config enabled it) simplified-mesh self-collision.
     *
     * The offset is passed through to the checker rather than applied here: composing it
     * needs pose maths in the TOOL frame, and the matching layer stays free of the robot
     * library (see WorldPickPose).
     *
     ...
     *
     * @param[in] obj                   matched object whose image position/angle is evaluated
     * @param[in] pickingOffset         pattern XYZ picking offset (mm), applied in the tool frame
     * @param[in] pickingRotationOffset pattern RX/RY/RZ picking offset (deg), applied in the tool frame
     * @return true when no checker is injected (advisory-only, does not gate matching),
     *         or the checker's own reachability/collision verdict
     */
    bool robotPossiblePickingCheck(const MatchedObject& obj,
...
private:
     ...
     * @brief One image contour that survived the coarse area filter and is a candidate to be
     *        edge-matched against one or more patterns.
     */
    struct PossibleObject {
    ...
    /**
     * @brief Non-SIMD edge-based template match: for every position, accumulates the normalized
     *        dot product between the pattern's gradient directions and the source image's Sobel
     *        gradient at that position, with an early-exit once the running partial score can no
     *        longer reach minScore.
     * @param[in]  matSrc    source pyramid-layer image to search (already rotated)
     * @param[in]  model     pattern providing the per-layer gradient template (getPatterns())
     * @param[out] matResult score map (CV_32FC1), sized (rows-tmplRows+1) x (cols-tmplCols+1)
     * @param[in]  layer     pyramid layer to match at
     * @param[in]  minScore  minimum score used both for early-exit and to accept a result
     * @param[in]  cfg       edge-match config (used for cfg.greediness)
    ...
    /**
     * @brief SIMD (AVX2) counterpart to MatchEdgePattern: same normalized gradient-dot-product
     *        score, computed via edge_template_match_simd_omp over 32-byte-aligned scratch
     *        buffers instead of the scalar early-exit loop.
     * @param[in]  matSrc    source pyramid-layer image to search
     * @param[in]  model     pattern providing the per-layer gradient template
     * @param[out] matResult score map (CV_32FC1)
     * @param[in]  layer     pyramid layer to match at
     * @param[in]  minScore  minimum score threshold
     * @return false if the template does not fit in matSrc or the pattern has no points at this layer
     */
    ...
    /**
     * @brief Finds the next-best match location in matResult after suppressing (setting to -1)
     *        the region around ptMaxLoc sized by (1 - maxOverlap), so a previously reported
     *        peak cannot be picked twice.
     * @param[in,out] matResult    score map, mutated in place (suppressed region filled with -1)
     * @param[in]     ptMaxLoc     location of the previous peak to suppress around
     * @param[in]     sizeTemplate template size, scaled by (1 - maxOverlap) to size the suppressed rect
     * @param[out]    maxValue     the next maximum score found
     * @param[in]     maxOverlap   suppression ratio: 0 suppresses just the template footprint
    ...
    /**
     * @brief Same suppression logic as the other overload, but also updates `blockMax`'s
     *        block-max acceleration structure over the suppressed region (used when the coarse
     *        search is running with BlockMax enabled).
     * @param[in,out] matResult    score map, mutated in place
     * @param[in]     ptMaxLoc     location of the previous peak to suppress around
     * @param[in]     sizeTemplate template size, scaled by (1 - maxOverlap)
     * @param[out]    maxValue     the next maximum score found
     * @param[in]     maxOverlap   suppression ratio
     * @param[in,out] blockMax     block-max structure updated over the suppressed region
     * @return location of the next maximum
     */
    cv::Point GetNextMaxLoc(cv::Mat& matResult, cv::Point ptMaxLoc,
    ...
    /**
     * @brief Refines a coarse match to sub-pixel/sub-degree precision by fitting a quadratic
     *        surface (least squares over a fixed 3x3x3 neighbourhood in x, y, and angle around
     *        iMaxScoreIndex) and solving for the surface's stationary point.
     * @param[in]  vec            candidate list; entry iMaxScoreIndex and its two angle-neighbours
     *                            supply the 27 fitted samples
     * @param[out] dNewX          refined X offset from the peak
     * @param[out] dNewY          refined Y offset from the peak
     * @param[out] dNewAngle      refined angle offset (degrees) from the peak
     * @param[in]  dAngleStep     angular spacing (degrees) between neighbouring angle samples
     * @param[in]  iMaxScoreIndex index of the peak candidate in vec
    ...
    /**
     * @brief Removes overlapping candidates: for every pair whose rotated rects intersect,
     *        drops the lower-scoring one (both if one fully contains the other, or if the
     *        intersection area exceeds dMaxOverLap fraction of rect i's area).
     * @param[in,out] vec         candidate list, mutated in place: entries marked _delete then erased
     * @param[in]     iMethod     comparison method; TM_SQDIFF keeps the lower score, any other
     *                            value (e.g. TM_CCOEFF_NORMED) keeps the higher score
     * @param[in]     dMaxOverLap max allowed overlap-area ratio before the lower-ranked candidate is dropped
     * @param[out]    del_indexes if non-null, receives the pre-erase indexes of every candidate
     *                            marked for deletion
     */
