...
/**
 * @brief Allocates the McContext subclass matching `frame_type` (only Frame_3E is
 *        currently implemented; other frame types return nullptr).
 * @param[in] frame_type the MC frame variant to build a context for
 * @param[in] data_code data encoding (binary/ASCII) passed to the Frame_3E context
