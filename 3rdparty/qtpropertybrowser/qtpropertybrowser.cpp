...
QString QtProperty::displayName() const
{
    return d_ptr->m_display_name;
}
...
void QtProperty::setDisplayName(const QString &text)
{
    if (d_ptr->m_display_name == text)
        return;

    d_ptr->m_display_name = text;
    propertyChanged();
}
