	...
	QtProperty* property, QWidget* parent)
{
    ...
    QStringList completerList = manager->completerStrings(property);
    if (!completerList.empty()) {
        editor->setCompleter(completerList);
    }
    ...
    connect(editor, &QtStringEdit::textEditingFinished, this, [property, manager](QString text) {
        manager->setInitialString(property, text);
    });

    connect(editor, SIGNAL(textEditingFinished(QString)),
            this, SLOT(slotSetValue(QString)));
